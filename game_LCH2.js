...
var actorChars = {
  ...
  "p": Powerup,
  "=": Lava, "|": Lava, "v": Lava, 
  "-": Block, "[": Block, "w": Block, // A platform that moves horizontally
  "i": Spike
};
...
// Wall is initialized based on the character, but otherwise has a
// size and position
function Block(pos, ch) {
  ...
  if (ch == "-") {
    ...
    // Drip block. Repeat back to this pos.
...
//When player lands on a moving block, player speed changes to block speed
/* if (Block) {
	level.playerTouched(Block);
	playerXSpeed = Block.speed;
}	
*/	
  ...
  if (type == "lava" && this.status == null) {
    ...
    if (!this.actors.some(function(actor) {
      ...
      this.finishDelay = 1;
		...
		playerXSpeed = 20; //(small number equals slower speed, larger number equals faster speed)
